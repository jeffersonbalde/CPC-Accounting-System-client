...
import { FaArrowDown, FaPlus, FaEye, FaFilter, FaSearch } from "react-icons/fa";
...
const Expenses = () => {
  ...
  const fetchExpenseTransactions = async () => {
    try {
      ...
      // Get all bills (they represent expenses)
      const billsResponse = await fetch(`${API_BASE_URL}/accounting/bills?per_page=100`, {
        headers: {
      ...
      if (billsResponse.ok) {
        ...
        // Transform bills to expense transactions
      ...
      // Get all journal entries to find manual expense entries
      ...
      // Filter journal entries that have expense accounts in debit lines
      allJournalEntries.forEach((entry) => {
        if (entry.lines && Array.isArray(entry.lines)) {
          // Find lines with expense accounts (debit side)
          entry.lines.forEach((line) => {
            if (
              line.account &&
              (line.account.account_type === "OPERATING_EXPENSES" ||
                line.account.account_type === "COST_OF_SERVICES") &&
              parseFloat(line.debit_amount) > 0
            ) {
              // Check if this entry is already included as a bill
              const isBillEntry = allTransactions.some(
                (t) => t.journal_entry_id === entry.id && t.type === "bill"
              );

              // Check if reference_number indicates it's a bill or invoice
              ...
              if (!isBillEntry && !isBillReference && !isInvoiceReference) {
                // This is a manual expense entry
                ...
                if (!existingTransaction) {
                  allTransactions.push({
                    ...
                    supplier_name: "", // Manual entries may not have supplier
                    amount: parseFloat(line.debit_amount) || 0,
                    description: line.description || entry.description || "Manual expense entry",
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/accounting/journal-entries`, {
        ...
        body: JSON.stringify({
          ...
          description: formData.description || `Expense: ${formData.expense_account_id}`,
      ...
      resetForm();
      // Refresh transactions to show the new manual entry
      await fetchExpenseTransactions();
    } catch (error) {
  ...
  // Group by account
  const expensesByAccount = expenseTransactions.reduce((acc, transaction) => {
    ...
    <div className="container-fluid px-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">Track all expense transactions</p>
        </div>
        <button
        ...
        <div className="col-md-4">
          <div className="card shadow">
            <div className="card-body">
              ...
              <h3 className="mb-0">{expenseTransactions.length}</h3>
            </div>
          </div>
        </div>
        <div className="col-md-4">
        ...
        <div className="col-md-4">
          <div className="card shadow">
            <div className="card-body">
              ...
              <h3 className="mb-0">{Object.keys(expensesByAccount).length}</h3>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Expenses by Account Summary */}
      {Object.keys(expensesByAccount).length > 0 && (
        <div className="card shadow mb-4">
          <div className="card-header bg-danger text-white">
            <h5 className="mb-0">Expenses by Account</h5>
          </div>
          <div className="card-body">
                    ...
                    .map((item, index) => (
                      <tr key={index}>
                        ...
                        <td className="text-end">
                          <strong className="text-danger">{formatCurrency(item.total)}</strong>
                        </td>
                      </tr>
                    ))}
                </tbody>
                ...
                {expenseTransactions.length === 0 ? (
                  ...
                  expenseTransactions.map((transaction) => (
                    <tr key={transaction.id}>
                      ...
                      <td className="text-end">
                        <strong className="text-danger">{formatCurrency(transaction.amount)}</strong>
                      </td>
                      <td>
                        {transaction.status && (
...
const ExpenseFormModal = ({ formData, setFormData, expenseAccounts, cashAccounts, suppliers, onSubmit, onClose }) => {
  return (
    <Portal>
      <div
    ...
    <Portal>
      ...
      >
        <div className="modal-dialog modal-lg">
          <div className="modal-content">
            <div className="modal-header bg-danger text-white">
              <h5 className="modal-title">Expense Transaction Details</h5>
              <button type="button" className="btn-close btn-close-white" onClick={onClose}></button>
            </div>
            <div className="modal-body">
              <div className="row mb-3">
              ...
              <div className="card">
                <div className="card-body">
                  <div className="text-center">
                    ...
                    <h2 className="text-danger mb-0">{formatCurrency(transaction.amount)}</h2>
                  </div>
                </div>
              </div>
              {transaction.status && (
